export function labelForUrl(url: string): string {
  ...
  if (
    /shop|store|market|магазин|wildberries|ozon|etsy|amazon|lamoda|manekenbrand|madj\.store|dropsstore|thegrezway|bananhot|adahlazorgan|wildflowercases|meundies|brooklinen|allbirds|awaytravel|limestore/.test(
      lower,
    )
  ) {
    return "Магазин";
